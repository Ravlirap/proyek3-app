...
class UserProvider with ChangeNotifier {
  ...
  // Getter untuk user profile (untuk Google Sign-In)
  ...
  // Set user profile (dipanggil setelah login/register dengan Google)
  void setUserProfile(Map<String, dynamic> user) {
    _userProfile = user;
    // Juga update field yang sesuai
    if (user['name'] != null) _name = user['name'];
    if (user['email'] != null) _email = user['email'];
    if (user['photoUrl'] != null) _profilePhotoUrl = user['photoUrl'];
    notifyListeners();
    debugPrint('User profile set: ${user['name']} - ${user['email']}');
  }
  
  // Clear user profile (dipanggil saat logout)
  ...
  // Get user name (untuk Google user)
  String getUserName() {
    return _userProfile?['name'] ?? _name;
  }
  
  // Get user email (untuk Google user)
  String getUserEmail() {
    return _userProfile?['email'] ?? _email;
  }
  
  // Get photo URL (untuk Google user)
  String? getPhotoUrl() {
    return _userProfile?['photoUrl'] ?? _profilePhotoUrl;
  }
